...
const TokenSetFromLoginPage = async(req,res)=>{

    console.log("entering this function: TokenSetFromLoginPage in controllers folder "+req.body.UsernameName +" "+req.body.PasswName)
    
    // import the JWT after installing it
    const jwt = require("jsonwebtoken");
  
   // CHECKING IF THE ENTERED NAME AND PASSWORD ARE ALREADY THERE IN DB
   let searcPassAndUsern = await usrDetailsDB.find({$and:[{username:req.body.UsernameName},{password:req.body.PasswName}]})
  ...
  let secretKey = require("crypto").randomBytes(64).toString("hex");
    ...
    let password = req.body.PasswName;
    ...
    secretKeyDB
      .updateMany({username:username,password:password},{$set:{ jwt: jToken, secretKey:secretKey}})
      .then(() => {
...
let userDerived = {userValue:"Welcome to Homepage "+user1}
  ...
  module.exports = TokenSetFromLoginPage